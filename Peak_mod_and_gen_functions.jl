...
function Rebuild1D(PeakfitCL, xdata, ydata, model)
    #= used to rebuild a fitted chromatogram peak by peak. 
    =#
    println("Rebuilding data...")
    FitDataCumaCL = zeros(size(xdata))
...
function TripleFitSLN1D(pt,xdata,ydata,NumOfPeaks)                                      #heb ik uit eindelijk niet gebruikt
    peaks_postsel = DataFrame()
    peaks_postsel.pn = [0]
    peaks_postsel.lin = [0.0]
    peaks_postsel.strt = [0.0]
   ...
   peaks_postsel =  vcat(peaks_postsel,pt)
   push!(peaks_postsel,(peaks_postsel.pn[end]+1, peaks_postsel.lin[end]+1, peaks_postsel.strt[end]+1, peaks_postsel.nd[end]+1, peaks_postsel.hgt[end]+1, peaks_postsel.wdt[end]+1, true))
...
function Adaptive1DSLN(pt,xdata,ydata,Ms,Es)                                            #SLN fitt
    #will perform either a single, double, or tripple SLN fit based on the proximity of 
    peaks_postsel = DataFrame()
    peaks_postsel.pn = [0]
    peaks_postsel.lin = [0.0]
   ...
   push!(peaks_postsel,(peaks_postsel.pn[end]+1, peaks_postsel.lin[end]+1, peaks_postsel.strt[end]+1, peaks_postsel.nd[end]+1, peaks_postsel.hgt[end]+1, peaks_postsel.wdt[end]+1, true))
    ...
    ydata = ydata .- minimum(ydata)
    println("Fitting data...")
    Peakfit = DataFrame(pn = [], fh = [], fp = [], fw = [], fm = [], fe = [], adj_r2 = [])
        ...
        for i = ProgressBar(2:size(peaks_postsel)[1]-1)
            ...
            if 1 .< curpn .< maximum(peaks_postsel.pn) 
                ...
                if sum(edge) .== 0
                    ...
                    p0 =    [peaks_postsel.hgt[i], peaks_postsel.lin[i], peaks_postsel.wdt[i], Ms[2], Es[2]]
                    ub =    [maximum(pt.hgt)*1.01, peaks_postsel.lin[i]*1.0005, peaks_postsel.wdt[i]*1.0005, Ms[3], Es[3]]
                    lb =    [minimum(pt.hgt)*0.99, peaks_postsel.lin[i]*0.9995, 0.00001, Ms[1], Es[1]]
                    mdl =   SLN
                elseif sum(edge) .== 1
                    TimeSelect = xdata[pt.strt[pt.pn .== curpn-1].<xdata.<pt.nd[pt.pn .==curpn]]
                    ...
                    ub =    [maximum(pt.hgt)*1.01, pt.lin[pt.pn.== curpn-1][1]*1.0005, pt.wdt[pt.pn.== curpn-1][1]*1.0005, Ms[3], Es[3], 
                    maximum(pt.hgt)*1.01, peaks_postsel.lin[i]*1.0005, peaks_postsel.wdt[i]*1.0005, Ms[3], Es[3]]
                ...
                elseif sum(edge) .== -2
                    ...
                    ub =    [maximum(pt.hgt)*1.01, peaks_postsel.lin[i]*1.0005, peaks_postsel.wdt[i]*1.0005, Ms[3], Es[3], 
                            maximum(pt.hgt)*1.01, pt.lin[pt.pn.== curpn+1][1]*1.0005, pt.wdt[pt.pn.== curpn+1][1]*1.0005, Ms[3], Es[3]]
            ...
            elseif curpn == 1
                try
                    ...
                    ub =    [maximum(pt.hgt)*1.01, peaks_postsel.lin[i]*0.9999, peaks_postsel.wdt[i]*1.0005, Ms[3], Es[3], 
                            maximum(pt.hgt)*1.01, peaks_postsel.lin[i]*1.000005, peaks_postsel.wdt[i]*1.0005, Ms[3], Es[3], 
                            maximum(pt.hgt)*1.01, pt.lin[pt.pn.== curpn+1][1]*1.000005, pt.wdt[pt.pn.== curpn+1][1]*1.0005, Ms[3], Es[3]]
                    ...
                    p0 =    [minimum(pt.hgt)*0.99,xdata[1]+0.00001,0.00001,Ms[2], Es[2],
                            peaks_postsel.hgt[i], peaks_postsel.lin[i], peaks_postsel.wdt[i], Ms[2], Es[2],
                            maximum(pt.hgt), mean(pt.lin), mean(pt.wdt), Ms[2], Es[2]]
        
                    ub =    [maximum(pt.hgt)*1.01, peaks_postsel.lin[i]*0.9999, 100, Ms[3], Es[3], 
                            maximum(pt.hgt)*1.01, peaks_postsel.lin[i]*1.000005, 100, Ms[3], Es[3], 
                            maximum(pt.hgt)*1.01, mean(pt.lin)*1.01, 100, Ms[3], Es[3]]
                    
                    lb =    [minimum(pt.hgt)*0.99, xdata[1], 0.00001, Ms[1], Es[1], 
                            minimum(pt.hgt)*0.99, peaks_postsel.lin[i]*0.999995, 0.00001, Ms[1], Es[1], 
                            minimum(pt.hgt)*0.99, mean(pt.lin)*0.99, 0.00001, Ms[1], Es[1]]
                    mdl =   TripleSLN
                end
            ...
            elseif curpn == maximum(peaks_postsel.pn)
                ...
                ub =    [maximum(pt.hgt)*1.01, pt.lin[pt.pn.== curpn-1][1]*1.000005, pt.wdt[pt.pn.== curpn-1][1]*1.0005, Ms[3], Es[3], 
                        maximum(pt.hgt)*1.01, peaks_postsel.lin[i]*1.000005, peaks_postsel.wdt[i]*1.0005, Ms[3], Es[3], 
                        maximum(pt.hgt)*1.01, xdata[end], peaks_postsel.wdt[i]*1.0005, Ms[3], Es[3]]
...
function findRoI1D(DataTrim, apex_th, peak_heigth_th;TimeTrim = TimeTrim)               #aandepaste functie die peak regios 
    ## usess local derivitve based local maxima finder to locate posible peak locations an outputs a table of peaks of interst
    ppks, pvals = findmaxima(DataTrim)                                                  #prelim peak finding using local maxima
    vpks, vvals = findminima(DataTrim)   
    vpks = [1; vpks; length(DataTrim)]                                             #""
...
function calcinput(Hgt, Wdt, m, As, Pos, gD, alpha)
    Max_Hgt = maximum(Hgt)
    Min_Hgt = minimum(Hgt)
    Max_Hgt_Outlier = ones(1, size(Pos, 2)) .* Max_Hgt .* 2
    Min_Hgt_Outlier = ones(1, size(Pos, 2)) .* Max_Hgt .* 0.1
    #Minimum & Maximum peak width? (in Min, max 5 min., Width measured at 1% Peak Height)
    Max_Wdt = median(Wdt) / 5.4989
    Min_Wdt = minimum(Wdt) / 5.4989
    #Required by Peakshape: 5-9, 12, 15-17.
    Max_m = maximum(m)
    Min_m = minimum(m)
    #Asymmetry parameter, Required by Peakshape: 6.
    Max_As = maximum(As)
    Min_As = minimum(As)
#=
    if length(Pos_1D) == 2
        angle  = tan(abs(Pos_2D[2]-Pos_2D[1])./abs(Pos_1D[2]-Pos_1D[1]))
        PeakDist = sqrt.((Pos_2D[2]-Pos_2D[1]).^2 + (Pos_1D[2]-Pos_1D[1]).^2)
    end
=#
    Peakdata = [Hgt'; Wdt'; Pos'; zeros(length(Hgt),1)'; m'; As'; gD'; alpha'];
    return Max_Hgt,
    Min_Hgt,
    Max_Hgt_Outlier,
    Min_Hgt_Outlier,
